...
void StudentTests(){
	...
	std::cout<<"Invalid splits"<<'\n';
	Rope invalid(IndexTestRope());
	Rope another;
	invalid.split(0,another);

	std::cout<<"Student Index finished"<<'\n';
	std::cout<<"Start report testing"<<'\n';
	Rope r5(IndexTestRope());
	std::string testingString = "Hello my name is Simon";
	std::string r5s;
	std::string ss5;
	r5.report(0,21,r5s);
	ss5 =  testingString.substr(0,22);
	assert(r5s==ss5);
	std::cout<<"Student report finished"<<'\n';

	std::cout<<"Student copy and assignment testing begin: "<<'\n';
	Rope ropeOneCopy(r1);
	std::cout<<"They should not have the same root"<<'\n';
	assert(ropeOneCopy.expose_root()->weight == r1.expose_root()->weight && ropeOneCopy.expose_root() != r1.expose_root());
	assert(ropeOneCopy.size() == r1.size());
	Rope copyR2 = r2;
	assert(copyR2.expose_root()->weight == r2.expose_root()->weight && copyR2.expose_root() != r2.expose_root());
	std::cout<<"Student copy and assignment operator test finished"<<'\n';
...
bool ValidRope(Node* root){
	...
	return (root->weight == left_sub) && (root->left) && ValidRope(root->right);
}

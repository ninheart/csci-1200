...
void MoreTests() {
  ...
  a.push_back("cat");
  a.push_back("dog");
  a.push_back("elephant");
  a.push_back("hippo");
  b.push_back("cat");
  b.push_back("dog");
  b.push_back("elephant");
  b.push_back("hippo");
  std::cout<<"Testing different types: string"<<'\n';
  assert(SAME(a,b));
  assert(a.size() == 4);
  a.print(std::cout);

  std::cout<<"Testing large set of data with push back and pop front"<<'\n';
  UnrolledLL<int> c;
  for(int i = 0; i < 150; ++i){
    c.push_back(i);
    c.pop_front();
  }
  std::cout<<"Since we are poping, the size of c should be same"<<'\n';
  c.print(std::cout);

  std::cout<<"Testing operator-- by printing a panlidrom"<<'\n';
  for(int i = 1; i < 21; ++i){
    c.push_back(i);
  }

  c.print(std::cout);
  UnrolledLL<int>::iterator it = c.begin();
  bool flag = true;
  while(flag){
    std::cout<<*it<<' ';
    it++;
  ...
  std::cout<<"Testing equal operator and copy constructor on list"<<'\n';
  UnrolledLL<int> d = c;
  std::cout<<"c and d should be the same"<<'\n';
  c.print(std::cout);
  d.print(std::cout);
  assert(c.size() == d.size());
  std::cout<<"Change the copy, c and d is not the same"<<'\n';
  d.push_back(101);
  d.push_back(102);
  assert(c.size() != d.size());
  std::cout<<"c size: "<<c.size()<<"  d size: "<<d.size()<<'\n';
  
  std::cout<<"Testing corner cases: creating an empty list "<<'\n';
  UnrolledLL<int> empty;
  assert(empty.empty());
  assert(empty.size() == 0);
  std::cout<<"Copying an empty list"<<'\n';
  UnrolledLL<int> another_empty = empty;
  assert(another_empty.size() == empty.size());

  std::cout<<"Student tests finished. ";
